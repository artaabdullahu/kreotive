...
(function () {
	...
	function doCharacterCount(evt) {
		...
		if ($('span#cke_maxlength_' + editor.name).length > 0) { // Check element exists
			setTimeout(function () {
				var charCount = editor.getData().replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').replace(/&\w+;/g ,'X').replace(/^\s*/g, '').replace(/\s*$/g, '').length;
...
// Plugin options
CKEDITOR.config.max_length = 20000;